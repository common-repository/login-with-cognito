...
/**
 * MOCognito Display Feedback Form
 */
function mocognito_display_feedback_form() {
	...
	wp_enqueue_style( 'mo_oauth_admin_settings_style', plugin_dir_url( dirname( __FILE__ ) ) . 'admin/css/style_settings.min.css', array(), '1.5.2' );
	...
	<script>
		...
		jQuery('a[aria-label="Deactivate <?php echo esc_attr( MO_OAUTH_README_PLUGIN_NAME ); ?>"]').click(function () {
			...
			jQuery('input:radio[name="deactivate_reason_radio"]').click(function () {
				...
				} else if (reason === "Have some custom requirements") {
					query_feedback.attr("placeholder", "Are you looking for any feature which is not already listed? Can you elaborate here?");
